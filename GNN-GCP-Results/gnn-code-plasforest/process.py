...
csvfiles = "/home/hp/FYP/GNN/gnn-code/plasforestresults"
_DEBUG = False
...
def process():
  ...
  for datafolder in os.listdir(directory):
      ...
      # try:
      feature_df = preprocess(csvfiles + "/" + datafolder + ".csv")
      ...
      y = torch.LongTensor(feature_df["class"].values)
      ...
      test_acc = test(model, data, test_set)
      print("Test Accuracy: ", test_acc, len(test_set))

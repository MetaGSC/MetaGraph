...
def process():
  ...
  for datafolder in os.listdir(directory):
      ...
      try:
        feature_df, find_count = preprocess(csvfiles + "/" + datafolder + ".fasta.probs.out")
      # feature_df = preprocess(csvfiles + "/" + datafolder + "_predictions.csv")
      except:
        continue

      node_features = torch.DoubleTensor(feature_df[["Probability"]].values)
      ...
      train_set, validate_set, test_set = get_train_validate_test_data(0.6, 0.1, feature_df["train_set"].values)
      ...
      pred_set = torch.BoolTensor(list(feature_df["Binary_Prediction"].values))

...
def process(userfolder, initpredictions, kmerfilepath, gfaprefix, pathfilesuffix, fastafilesuffix, fileseparator, isKmerAvailable, result_df):
  data = None
  ...
  # model = get_model(inputfeatures = data.x.shape[1], hidden_channels = [4], num_classes = 2)
  print(model)
  ...
  if not(isKmerAvailable):
    result_df = getPrecisionRecall(ground_df["node_id"], ground_df["class"], find_set, binary_predictions_set, True, userfolder, result_df, isKmerAvailable)
  
  valid_pred = updatePreds(pred, binary_predictions_set, find_set)

  result_df = getPrecisionRecall(ground_df["node_id"], ground_df["class"], find_set, valid_pred, False, userfolder, result_df, isKmerAvailable)
